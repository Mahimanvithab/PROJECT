...
public class ArrayListCalculator1 {
    ...
    private static class ParenthesisResult {
        ...
        ParenthesisResult(double result, ArrayList<String> expression) {
            this.result = result;
    ...
    public static void runCalculator(Scanner sc) {
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 5: // Show Result
                    System.out.println("Expression: " + String.join("", expression) + " = " + formatNumber(result));
                ...
                case 7: // Parenthesis Expression
                    int opChoice = chooseParenthesisOperator(sc);
                    if (opChoice == 9) break;
                    ParenthesisResult pr = handleParenthesis(sc);
                    if (pr.expression.isEmpty()) break;
                    switch (opChoice) {
                        case 1: result += pr.result; expression.add(" + ("); break;
                        case 2: result -= pr.result; expression.add(" - ("); break;
                        case 3: result *= pr.result; expression.add(" * ("); break;
                        case 4:
                            if (pr.result == 0) {
                            ...
                            expression.add(" / (");
                            break;
                        case 5: result = Math.pow(result, pr.result); expression.add(" ^ ("); break;
                        case 6:
                            if (pr.result == 0) {
                ...
                case 11: // Factorial
                    if (expression.size() >= 3) {
                        String lastNumStr = expression.get(expression.size() - 1);
                        try {
    ...
    private static ParenthesisResult handleParenthesis(Scanner sc) {
        ...
        while (true) {
            ...
            if (choice == 6) return new ParenthesisResult(0, new ArrayList<>());
    ...
    private static String formatNumber(double num) {
        return (num == (long) num) ? String.valueOf((long) num) : df.format(num);
    }
}